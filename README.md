...
## Pre-requisites
To run the covid_xprize.examples, you will need:
...
## Examples
Under the `covid_xprize.examples` directory you will find some covid_xprize.examples of predictors and prescriptors that you can 
...
In order to run the covid_xprize.examples locally:
1. Ensure your current working directory is the root folder of this repository (the same directory as this README 
resides in). The covid_xprize.examples assume your working directory is set to the project root and all paths are relative to 
it.
...
1.  Browse to and launch one of the covid_xprize.examples (such as `linear`) and run through the steps in the associated 
...
1. Expect your script to be called as follows (the dates and filenames are just covid_xprize.examples and will vary):
    ```shell script
...
To use it, copy `covid_xprize.examples/predictors/lstm/tests/fixtures/trained_model_weights_for_tests.h5` to `covid_xprize.examples/lstm/models/trained_model_weights.h5`,
...
covid_xprize.examples of how to make this call.
