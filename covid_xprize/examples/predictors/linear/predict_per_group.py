...
MODEL_FILE = os.path.join(ROOT_DIR, "models", "lars.pkl")
...
def predict_df(start_date_str: str, end_date_str: str, path_to_ips_file: str, verbose=False):
    ...
    curr = df.Date.max() + np.timedelta64(1, 'D')
    ips_df = hist_ips_df[(hist_ips_df.Date >= curr) & (hist_ips_df.Date <= end_date)]
    ...
    for key, value in df.groupby("GeoID"):
        ...
        features = X[-LAGS:].tolist()
        try:
            m = MODELS[key]
        except KeyError:
            _ = gips_df.loc[:, id_cols].copy()
            _['PredictedDailyNewCases'] = 0
            R.append(_)
            continue
        HY = []
        for i, _ in enumerate(pd.date_range(curr, end_date)):
            hy = m.predict(np.atleast_2d(np.array(features).flatten()))[0]
            hy = min(hy, 12)
            _ = gips_np[i].tolist()
            _.append(hy)
            del features[0]
            ...
            HY.append(hy)
        _ = gips_df.loc[:, id_cols].copy()
        _['PredictedDailyNewCases'] = np.exp(HY)
        R.append(_)
